...
public class WheelView extends View {
    ...
    private void initResourcesIfNecessary() {
        if (itemsPaint == null) {
            ...
            itemsPaint.setTextSize(DisplayUtil.INSTANCE.sp2px(context, TEXT_SIZE));
        }
        //选中值字体大小 样式
        if (valuePaint == null) {
            valuePaint = new TextPaint(Paint.ANTI_ALIAS_FLAG | Paint.DITHER_FLAG);
            valuePaint.setTextSize(DisplayUtil.INSTANCE.sp2px(context, TEXT_SIZE));
            valuePaint.setShadowLayer(0.2f, 0, 0.2f, 0xFFC0C0C0);
        }
        //选中颜色
        if (centerDrawable == null) {
            centerDrawable = getContext().getResources().getDrawable(R.color.tran);
        }
    ...
    private int getDesiredHeight(Layout layout) {
        ...
        int desired = getItemHeight() * visibleItems - DisplayUtil.INSTANCE.dip2px(context,ITEM_OFFSET * 2)
                - DisplayUtil.INSTANCE.dip2px(context,ADDITIONAL_ITEM_HEIGHT);
    ...
    private int calculateLayoutWidth(int widthSize, int mode) {
        ...
        if (mode == MeasureSpec.EXACTLY) {
            ...
            width = itemsWidth + labelWidth + 2 * DisplayUtil.INSTANCE.dip2px(context,PADDING);
            if (labelWidth > 0) {
                width += DisplayUtil.INSTANCE.dip2px(context,LABEL_OFFSET);
            }
        ...
        if (recalculate) {
            // recalculate width
            int pureWidth = width - DisplayUtil.INSTANCE.dip2px(context,LABEL_OFFSET) - 2 * DisplayUtil.INSTANCE.dip2px(context,PADDING);
            if (pureWidth <= 0) {
            ...
            if (labelWidth > 0) {
                ...
                itemsWidth = pureWidth + DisplayUtil.INSTANCE.dip2px(context,LABEL_OFFSET); // no label
            }
        }
    ...
    private void createLayouts(int widthItems, int widthLabel) {
        if (itemsLayout == null || itemsLayout.getWidth() > widthItems) {
            itemsLayout = new StaticLayout(buildText(isScrollingPerformed), itemsPaint, widthItems,
                    ...
                    1, DisplayUtil.INSTANCE.dip2px(context,ADDITIONAL_ITEM_HEIGHT), false);
        } else {
        ...
        if (widthLabel > 0) {
            if (labelLayout == null || labelLayout.getWidth() > widthLabel) {
                labelLayout = new StaticLayout(label, valuePaint,
                        ...
                        DisplayUtil.INSTANCE.dip2px(context,ADDITIONAL_ITEM_HEIGHT), false);
            } else {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (itemsWidth > 0) {
            ...
            canvas.translate(DisplayUtil.INSTANCE.dip2px(context,PADDING), -DisplayUtil.INSTANCE.dip2px(context,ITEM_OFFSET));
            drawItems(canvas);
            drawValue(canvas);
            canvas.restore();
        }
    ...
    private void drawValue(Canvas canvas) {
        ...
        if (labelLayout != null) {
            canvas.save();
            canvas.translate(itemsLayout.getWidth() + DisplayUtil.INSTANCE.dip2px(context,LABEL_OFFSET), bounds.top);
